...
| Creative — video **multimodal** (picture+audio+timing) | a video-capable hosted model (e.g. `gemini-3.7-flash` via OpenRouter) | — | monthly | — | Reads fine print, dense UI and caption timing that frame sampling cannot resolve. **Split the authority: local Whisper is ground truth for what was SAID, the multimodal model for what was SHOWN.** These models can carry on-screen text into a transcript — verify any claim about speech. Running both is the cross-check. |
