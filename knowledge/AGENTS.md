...
- **`person`** — an individual — client-side or your own side. Lives in `wiki/`, not a separate `people/` folder (see above).
...
This template's `knowledge/` folder had `wiki/`, `decisions/`, `people/`, and `sources/` but no orientation file — nothing telling a session how the folder is meant to be used, or that it's the capsule schema itself rather than a build-log of this repo. Added the canonical `knowledge/AGENTS.md` + thin `knowledge/CLAUDE.md` pointer pattern used across every capsule.

### 2026-07-12 — Brought current to dotKnowledge SPEC.md, ahead of real capsule builds
Audited against the real spec (github.com/polychrome-studio/dotKnowledge) ahead of stamping real bundles. Findings and fixes:
- **`cartridge.yaml` → `capsule.yaml` was a rename debt, not just a rename.** An earlier cartridge→capsule terminology sweep never touched this repo's manifest — it was still `cartridge.yaml`, and its schema had independently drifted from SPEC §5 (missing `type:`/`relationship:`/`parent:`/`access:` entirely, `format:`/`version:` using different keys than `format_version:`, `rises:` as free prose instead of the 3-value enum, `legal_signoff` nested under a `boundary:` key the real spec doesn't have). Rebuilt against §5 directly, kept a `contents:` block as an explicit, additive extension (permitted per §8). Swept the "cartridge" prose across README/AGENTS.md/SCOPE.md/design.md/skills-README (word-boundary-safe).
